...
| 2 | **Infra provisioning** | Run `infra/setup.sh` end to end in a real project: bucket + lifecycle rule, Pub/Sub topics/subscriptions, BigQuery dataset/table. Triggering is manual (`scripts/trigger_phase1.py`) — no GCS notification fires on upload | 0.5–1 day | Not started |
...
| 10 | **Full 130k backfill** | Upload the corpus, then run `scripts/trigger_phase1.py` to start the batch and monitor it. Active engineering time is low, but wall-clock run time depends heavily on the legacy/scanned-doc mix — OCR and LibreOffice conversion are the slow paths. Ballpark: 4 hours to ~2 days of processing time with reasonable Cloud Run concurrency | 0.5 day active + up to 2 days passive | Not started |
